...
<section class="movie-trend">
    <c:if test="${not empty trendFilms}">
        ...
        <div style="width: 50%; padding-top: 50px; padding-left: 120px;" class="text-white my-4">
            ...
            <div class="buttons text-light">
                <a href="playlist?id=${trendFilms.getIdFilm()}" class="btn text-light mt-2">Add to playlist <i class='bx bxs-right-arrow'
                                                                    style='color:#ffffff'></i></a>

                <a href="view-now?id=${trendFilms.getIdFilm()}" class="btn btn-open  text-light mt-2 " >More details <i class='bx bxs-show'
                                                                                style='color:#ffffff'></i></a>
...
<c:if test="${not empty recommendedFilm}">
    <section style="height: 300px; padding-left: 50px; margin-bottom: 220px;" class="movie-card-section">
        <div id="slider">
            <p style="font-size: 20px; text-align: start; font-weight: bold;" class="text-light">Recommendation</p>
            <div class="d-flex flex-row">
                <div style="width: 98%;" class="cards row">
                    <div class="swiper">
                        <div style="" class="swiper-wrapper cards row">
                            <c:forEach var="film" items="${recommendedFilm}" varStatus="loop">
                                <div class="movie-card-trend text-white">
                                    <a href="s?id=${film.getIdFilm()}">
                                        <img style="height: 220px; width: 150px; margin-top: 20px;" class="card" src="${film.getPictureURL()}" alt="${film.getTitleFilm()}">
                                    </a>
...
<section  class="showtimes" style="height: 100vh; width: 100%; background-color: black; padding-top: 40px; padding-left: 50px;" >
    ...
    <div >
        <a class="activeFilm text-light" style="font-weight:500; margin-left: 20px; padding: 7px;" onclick="showSection('all')">All</a>
        <a class="text-light" style="margin-left: 20px; font-weight: 500; padding: 7px;" onclick="showSection('showingNow')">Showing Now</a>
        <a class="text-light" style="margin-left: 20px; font-weight: 500; padding: 7px;" onclick="showSection('comingSoon')">Coming Soon !</a>
    </div>
    <div class="row all">
    ...
    <div class="row showingNow">
        <c:forEach var="film" items="${ShowingNow}">
            <div  class="col-md-4">
    ...
    <div class="row comingSoon">
        <c:forEach var="film" items="${ComingSoon}">
            <div  class="col-md-4">
...
<script>
    function showSection(sectionName) {
        var allSection = document.querySelector('.row.all');
        var showingNowSection = document.querySelector('.row.showingNow');
        var comingSoonSection = document.querySelector('.row.comingSoon');

        var activeFilmLink = document.querySelector('.activeFilm');
        activeFilmLink.classList.remove('activeFilm');

        if (sectionName === 'all') {
            allSection.style.display = 'flex';
            showingNowSection.style.display = 'none';
            comingSoonSection.style.display = 'none';
        } else if (sectionName === 'showingNow') {
            allSection.style.display = 'none';
            showingNowSection.style.display = 'flex';
            comingSoonSection.style.display = 'none';
        } else if (sectionName === 'comingSoon') {
            allSection.style.display = 'none';
            showingNowSection.style.display = 'none';
            comingSoonSection.style.display = 'flex';
        }

        event.target.classList.add('activeFilm');
    }
